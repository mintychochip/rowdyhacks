  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 data-mobile-h1 style={{ ...TYPO.h1, margin: 0 }}>Judging Results</h2>
        <div style={{ display: 'flex', gap: SPACE.sm }}>
          <button onClick={() => navigate(`/hackathons/${hackathonId}/judging`)}
            style={{
              padding: '8px 16px', background: 'none', border: `1px solid ${BORDER}`,
              borderRadius: 6, color: TEXT_SECONDARY, cursor: 'pointer', fontSize: 13,
            }}>
            Judge Portal
          </button>
          <button onClick={loadResults}
            style={{
              padding: '8px 16px', background: PRIMARY, border: 'none', borderRadius: 6,
      ...
      {loading ? (
        ...
        <div style={{
          padding: 24, background: INPUT_BG, border: `1px solid ${BORDER}`,
          borderRadius: 12, textAlign: 'center',
        }}>
          <p style={{ color: TEXT_MUTED }}>{error}</p>
          <button onClick={loadResults}
            style={{
              marginTop: 12, padding: '8px 20px', background: PRIMARY, border: 'none',
        ...
        <>
          {/* Rankings */}
          <div style={{ background: INPUT_BG, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden', marginBottom: SPACE.lg }}>
            <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                ...
                <tbody>
                  {rankings.map((r) => {
                    ...
                    return (
                      <tr key={r.submission_id} style={{ borderBottom: `1px solid ${BORDER}` }}>
                        ...
                        <td style={{ padding: '10px 16px', textAlign: 'right', color: TEXT_SECONDARY }}>
                          {r.raw_avg}
                        </td>
                  ...
                  {rankings.length === 0 && (
                    <tr>
                      <td colSpan={6} style={{ textAlign: 'center', padding: SPACE.xl - 16, color: TEXT_MUTED }}>
          ...
          {/* Judge Stats */}
          {judgeStats.length > 0 && (
            <div style={{ background: INPUT_BG, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden' }}>
              <h3 style={{
                fontSize: 14, padding: `${SPACE.sm + 4}px ${SPACE.md}px`,
                color: TEXT_MUTED, margin: 0, borderBottom: `1px solid ${BORDER}`,
              }}>
                Judge Analysis
              </h3>
              <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${BORDER}`, textAlign: 'left' }}>
                      ...
                      <th style={{ padding: '8px 16px', color: TEXT_MUTED, fontWeight: 500, textAlign: 'right' }}>Severity (+ generous)</th>
                      <th style={{ padding: '8px 16px', color: TEXT_MUTED, fontWeight: 500, textAlign: 'right' }}>Precision</th>
                      <th style={{ padding: '8px 16px', color: TEXT_MUTED, fontWeight: 500, textAlign: 'right' }}>Noise (&sigma;)</th>
                    ...
                    {judgeStats.map(j => (
                      <tr key={j.judge_id} style={{ borderBottom: `1px solid ${BORDER}` }}>
                        ...
                        <td style={{
                          ...
                          fontFamily: TYPO['mono-data'].fontFamily,
                          color: j.severity > 0 ? SUCCESS : ERROR,
                        }}>
                          {j.severity > 0 ? '+' : ''}{j.severity}
                        </td>
                        <td style={{ padding: '8px 16px', textAlign: 'right', fontFamily: TYPO['mono-data'].fontFamily, color: TEXT_PRIMARY }}>
                          {j.precision}
                        </td>
                        <td style={{ padding: '8px 16px', textAlign: 'right', fontFamily: TYPO['mono-data'].fontFamily, color: TEXT_MUTED }}>
                          {j.sigma}
                        </td>

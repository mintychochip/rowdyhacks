...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE documenttype AS ENUM ('hackathon_info', 'track_info', 'faq', 'schedule', 'rules', 'resources', 'submission_summary')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "streaming", "completed", "error", name="assistantmessagestatus"),
            nullable=False,
        ),

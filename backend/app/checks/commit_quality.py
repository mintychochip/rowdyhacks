...
SUSPICIOUS_MESSAGES = {
    "update", "fix", "commit", "test", "wip", ".", "changes",
    "stuff", "done", "work", "push", "save", "final",
    "initial commit", "first commit", "added files",
    "updated code", "fixed bug", "minor fix", "clean up",
    "cleanup", "refactor", "updated", "fixed", "added",
    "patch", "tweak", "tmp", "temp", "misc",
}
...
async def check_commit_quality(context: CheckContext) -> CheckResult:
    ...
    if total == 0:
        return CheckResult(
            check_name="commit-quality", check_category="timeline",
            score=30, status="warn", details={"reason": "No commits found"},
        )
    ...
    if pct_bad > 80 and total >= 3:
        score += 35
        evidence.append(f"{pct_bad}% of {total} commits are 'update', 'fix', or similar — likely AI-generated or copy-pasted")
    ...
    conventional = sum(1 for m in messages if re.search(r'^(feat|fix|chore|docs|style|refactor|test|perf|ci|build|revert)[(:]', m))

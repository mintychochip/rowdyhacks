...
async def test_forensics_future_commit(tmp_path):
    ...
    (tmp_path / "test.txt").write_text("hello")
    subprocess.run(["git", "add", "."], cwd=tmp_path, capture_output=True)
    subprocess.run(["git", "commit", "-m", "future", f"--date={future_str}", "--no-verify"], cwd=tmp_path, capture_output=True)
...
async def test_forensics_date_mismatch(tmp_path):
    ...
    (tmp_path / "test.txt").write_text("hello")
    subprocess.run(["git", "add", "."], cwd=tmp_path, capture_output=True)
    subprocess.run(["git", "commit", "-m", "backdated", f"--date={past_str}", "--no-verify"], cwd=tmp_path, capture_output=True)
